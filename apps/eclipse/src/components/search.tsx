...
export default function EclipseSearchDialog({ open, onOpenChange }: SharedProps) {
  if (!open) return null;

  return createPortal(
    <SearchDialogInner onClose={() => onOpenChange(false)} />,
    document.body,
  );
}
    ...
    <div className="fixed inset-0 z-[60] flex items-start justify-center pt-[15vh]" onClick={onClose}>
      ...
      >
        ...
        <div className="px-4 h-12 flex flex-row items-center gap-2 border-b last:border-b-none border-stroke-neutral">
          ...
          />
          <kbd
            className="px-2 h-6 flex flex-row items-center bg-background-neutral font-mono text-xs text-foreground-neutral-weaker rounded-square-low cursor-pointer"
            onClick={onClose}
          >
            ESC
          </kbd>
